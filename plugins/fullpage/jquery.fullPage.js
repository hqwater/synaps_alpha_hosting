	...
	$.fn.fullpage = function(options) {
		...
		*/
		...
		$('.section').each(function(index){
			...
			//$(this).css('height', windowsHeight + '.7px');
		...
		*/		
		...
		function moveSlide(direction){
		    ...
		    //isn't there a next slide in the secuence? (disabled 비활성)
			if(!destiny.length){
		...
		/**
		 * Scrolling horizontally when clicking on the slider controls. (modified 수정됨)
		 */
		...
		$('#nav_h').on('click', '.indicator', function() {
			if ($(this).hasClass('prev')) {
				$.fn.fullpage.moveSlideLeft();
			} else {
				$.fn.fullpage.moveSlideRight();
		...
		*/
		...
		function doneResizing() {
			...
			$('.section').each(function(){
				...
				if(options.verticalCentered){
				//	$(this).find('.tableCell').css('height', getTableHeight($(this)) + '.9px');
				}
				
				//$(this).css('height', windowsHeight + '.8px');
		...
		*/
		...
		function addTableClass(element){
			// element.addClass('table').wrapInner('<div class="tableCell" style="height:' + getTableHeight(element) + '.2px;" />');
			element.addClass('table').wrapInner('<div class="tableCell" />');
		}
